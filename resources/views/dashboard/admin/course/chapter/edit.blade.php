...
@section('content')
    <div class="container">
        <div class="card p-5">
            ...
            <div class="card-body">
                <form action="/admin/chapter/{{ $chapter->id }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row mb-3">
                        <div class="col-6">
                            ...
                            <select name="course_id" class="form-select select_box tipe">
                                ...
                                @forelse ($courses as $course)
                                    @if (old('course_id') || $chapter->course_id)
                                        @if (old('course_id') == $course->id || $chapter->course_id == $course->id)
                        ...
                        <div class="col-6">
                            ...
                            <input type="number" class="form-control px-4" min="1" id="ordinal" name="ordinal"
                                value="{{ old('ordinal') ?? $chapter->ordinal }}">
                            @error('ordinal')
                                <div class="error">*{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="short_desc" id="short_desc">{{ old('short_desc') ?? $chapter->short_desc }}</textarea>
                        @error('short_desc')
                            <div class="error">*{{ $message }}</div>
                        @enderror
                    </div>
                    <div>
                        ...
                        <textarea class="form-control" name="content" id="summernote">{{ old('content') ?? $chapter->content }}</textarea>
                        @error('content')
                            <div class="error">*{{ $message }}</div>
                        @enderror
                    </div>
